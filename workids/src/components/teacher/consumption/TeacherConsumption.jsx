...
export default function TeacherConsumption() {
  ...
  const outStadndingItems = outStandingConsumptionList.map((menu, index) => (
    <div
      key={index}
      className="row justify-content-md-center p-1"
      style={{ fontSize: "18px", textAlign: "center" }}
    >
      <div className="col-sm-1 p-2" style={colStyle}>
        {menu.citizenNumber}
      </div>
      <div className="col-sm-2 p-2" style={colStyle}>
        {menu.studentName}
      </div>
      <div className="col-sm-3 p-2" style={{ ...colStyle, overflow: "hidden" }}>
        {menu.content}
      </div>
      <div className="col-sm-1 p-2" style={colStyle}>
        {menu.amount}
      </div>
      <div className="col-sm-3 p-2" style={colStyle}>
        {menu.createdDate}
      </div>
      <div className="col-sm-2 p-2" style={colStyle}>
        <TeacherConsumptionProcess
          consumptionNationStudentNum={menu.consumptionNationStudentNum}
          state={menu.state}
        />
      </div>
    </div>
    ...
    <div style={divStyle} className="border border-dark  border-3 p-3">
      ...
      {state === 0 ? (
                ...
                {consumptionList.map((menu, index) => (
                  <tbody key={index} style={{ fontSize: "20px", height: "15vh" }}>
                    ...
                    >
                      ...
                      <td>
                        <TeacherConsumptionUpdate
                          consumptionNum={menu.consumptionNum}
                          content={menu.content}
                          amount={menu.amount}
                        />
                      </td>
                      <td>
                        <TeacherConsumptionDelete consumptionNum={menu.consumptionNum} />
                      </td>
                    </tr>
        ...
        <>
          ...
          <div
            style={firstBlock}
            className="container justify-content-md-center border border-dark  border-3"
          >
            ...
            >
              <div className="col-sm-1 p-1" style={colStyle}>
                번호
              </div>
              <div className="col-sm-2 p-1" style={colStyle}>
                이름
              </div>
              <div className="col-sm-3 p-1" style={{ ...colStyle, overflow: "hidden" }}>
                소비내역
              </div>
              <div className="col-sm-1 p-1" style={colStyle}>
                금액
              </div>
              <div className="col-sm-3 p-1" style={colStyle}>
                승인일
              </div>
            ...
            <div
              className="scrollCss"
              style={{ height: "70%", overflowX: "hidden", overflowY: "auto" }}
            >
              {approvalItems}
            </div>
          </div>
